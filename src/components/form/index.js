...
export default function Form() {
      ...
      <>
        {bolsa.length === 0 ? (
          <div className="mt-5 text-center">
            {' '}
            Dados não confere para coleta de dados altere a data ou o NIS/CPF
          </div>
        ) : (
          bolsa.map((bol) => {
            return (
  ...
  async function handleSubmit(e) {
    ...
    const respose = await axios.get(
      `http://www.transparencia.gov.br/api-de-dados/bolsa-familia-disponivel-por-cpf-ou-nis?codigo=${nis}&anoMesReferencia=${year}${mouth}&anoMesCompetencia=${year}${mouth}&pagina=1`
    );
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-group">
          ...
          <input
            ...
            placeholder="CPF/NIS (sem máscara, somente números)"
          />
        </div>
        <div className="form-group">
          <label>Ano</label>
